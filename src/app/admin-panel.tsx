...
export default function AdminPanel() {
    ...
    const handleApprove = async (userId: number) => {
        try {
            ...
            if (response.ok) { Alert.alert('Approved', 'Course rep account has been approved.'); fetchPendingUsers(); }
        } catch { Alert.alert('Error', 'Failed to approve user.'); }
    };
    ...
    const handleReject = async (userId: number) => {
        try {
            ...
            if (response.ok) { Alert.alert('Rejected', 'Course rep request has been rejected.'); fetchPendingUsers(); }
        } catch { Alert.alert('Error', 'Failed to reject user.'); }
    };
        ...
        <SafeAreaView style={styles.safeArea}>
            <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.noticeCard}>
                    ...
                    <Text style={styles.noticeText}>You have access to all features including course rep approval and all academic tools.</Text>
                </View>
                ...
                <View style={styles.section}>
                    ...
                    {pendingUsers.length === 0 ? (
                        <View style={styles.emptyCard}>
                            ...
                            <Text style={styles.emptyText}>When students register as course reps, their requests will appear here.</Text>
                        </View>
                    ) : (
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.actionList}>
                        {[
                            { title: '📢 Post Announcement', text: 'Share class updates, cancellations, venue changes, and reminders.', route: '/post-announcement' },
                            { title: '🗓️ Manage Timetable', text: 'Add or update class times, course titles, lecturers, and venues.', route: '/manage-timetable' },
                            { title: '📝 Add Assignment', text: 'Record assignment details, due dates, and submission instructions.', route: '/add-assignment' },
                            { title: '📍 Upload Exam Venue Info', text: 'Add exam venues so students can search and prepare early.', route: '/manage-exam-venues' },
                            ...
                            { title: '🔍 Exam Venue Search', text: 'Find exam venues using index or reference number.', route: '/exam-venue-search' },
                        ].map((item) => (
